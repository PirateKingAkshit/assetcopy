  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const statusResponse = await axiosInstance.get('status/all')
        ...
        const conditionResponse = await axiosInstance.get('condition/all')
        ...
        const departmentResponse = await axiosInstance.get('dept/all')
...
useEffect(() => {
  if (formData.depreciableAsset === 'yes' && formData.capitalizationDate) {
    setFormData(prev => ({
      ...prev,
      insuranceStartDate: prev.capitalizationDate
    }))
  } else if (formData.depreciableAsset === 'no') {
    setFormData(prev => ({
      ...prev,
      insuranceStartDate: null // ✅ reset insurance start date
    }))
  }
}, [formData.capitalizationDate, formData.depreciableAsset])


  //asset code
  useEffect(() => {
    // Fetch next asset code on component mount
    const fetchNextAssetCode = async () => {
      try {
        const response = await axiosInstance.get('/asset/next-code')
        if (response.data.status === 200) {
          setFormData(prev => ({ ...prev, assetCode: response.data.data.nextAssetCode }))
        }
      } catch (error) {
  ...
  if (section === 'asset_details') {
    newErrors.asset_details = {
      ...
      file:
        formData.file && formData.file.size > 5 * 1024 * 1024
    ...
    } else if (formData.warrantyStatus === 'insurance') {
      // ✅ Insurance Start Date Validation
      if (!formData.insuranceStartDate) {
        newErrors.warranty_info.insuranceStartDate = 'Insurance Start Date is required'
      } else {
        const capitalizationDate = formData.capitalizationDate ? dayjs(formData.capitalizationDate) : null
        const insuranceStartDate = formData.insuranceStartDate ? dayjs(formData.insuranceStartDate) : null

        if (insuranceStartDate && capitalizationDate && insuranceStartDate.isBefore(capitalizationDate, 'day')) {
          newErrors.warranty_info.insuranceStartDate = `Insurance Start Date cannot be earlier than Capitalization Date (${capitalizationDate.format('DD-MM-YYYY')})`
        } else {
          newErrors.warranty_info.insuranceStartDate = ''
        }
      }

      // Clear other warranty fields
  ...
  if (hasAssetErrors || hasWarrantyErrors) {
  ...
  try {
    ...
    if (formData.warrantyStatus === 'amc') {
      formDataToSend.append('amc_vendor', formData.amcVendor);
                  ...
                  <Grid item xs={12} sm={4}>
                    <FormControl fullWidth required error={!!errors.purchase_info?.vendorName}>
                      <Autocomplete
                        ...
                        onChange={(e, newValue) =>
                          setFormData({ ...formData, vendorName: newValue ? newValue._id : '' })
                        }
                        renderInput={params => (
                          <TextField
                            {...params}
                    ...
                    <Grid container spacing={5}>
                      <Grid item xs={12} sm={4}>
                        <TextField
                          ...
                          onChange={e => setFormData({ ...formData, amcVendor: e.target.value })}
                      ...
                      <Grid item xs={12} sm={4}>
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                          <DatePicker
                            ...
                            required
                            onChange={value => {
                              setFormData({ ...formData, insuranceStartDate: value })
                            ...
                            slotProps={{
                              textField: {
                                fullWidth: true,
                                error: !!errors.warranty_info?.insuranceStartDate,
                                helperText: errors.warranty_info?.insuranceStartDate,
                                placeholder: 'DD-MM-YYYY'
                              }
                            }}
                          />
