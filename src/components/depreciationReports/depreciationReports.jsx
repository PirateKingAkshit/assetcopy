...
const Depreciationreports = () => {
  ...
  const staticColumnDefs = useMemo(
    () => [
      ...
      {
        headerName: 'Capitalization Amount',
        field: 'capitalization_amount',
        cellStyle: {
          display: 'flex',
          alignItems: 'center',
          padding: '0 10px',
      ...
      {
        headerName: 'Opening Accumulated Depreciation',
      ...
      {
        headerName: 'Opening Net Balance',
        field: 'opening_net_balance',
        cellStyle: {
          display: 'flex',
          alignItems: 'center',
          padding: '0 10px',
      ...
      {
        headerName: 'Closing Net Balance',
        field: 'closing_net_balance',
        cellStyle: {
          display: 'flex',
          alignItems: 'center',
          padding: '0 10px',
  ...
  const generateReport = async () => {
    try {
      ...
      const response = await axiosInstance.post('/depriceationReportRoute', payload)
      ...
      if (response.data.status === 200) {
        ...
        setRowData(apiData)
        setDynamicColumnDefs(dynamicCols)
  ...
  try {
    ...
    // ✅ Use dayjs for timestamp in filename
    const fileName = `Depreciation_Report_${dayjs().format('YYYYMMDD_HHmmss')}.xlsx`;
  ...
  return (
    <Card className='flex flex-col'>
      <CardHeader title='Depreciation Report Table' />
      {error && (
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4'>
          ...
          <FormControl fullWidth size='small' disabled>
            ...
            <Select value={clientMethod} label='Depreciation Method'>
              <MenuItem value={clientMethod || ''}>{clientMethod || 'N/A'}</MenuItem>
            </Select>
          </FormControl>
...
{/* financial start month */}
        ...
        {reportData.length > 0 && (
          <div>
            <AgGridWrapper
              rowData={reportData}
              columnDefs={dynamicColumnDefs.length > 0 ? dynamicColumnDefs : staticColumnDefs}
              gridRef={gridRef}
