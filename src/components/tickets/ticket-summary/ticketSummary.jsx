...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.data?.status === 200) {
      ...
      const newStatus = statusOptions.find(s => s._id === status)?.status
      setTimeout(() => {
        if (newStatus === 'close') {
          router.push(`/${locale}/ticket/ticket-list`)
        } else {
    ...
    // 👇 Ab yahan backend ka message show hoga
    const errorMessage =
  ...
  return (
    <Card className='p-6 space-y-4'>
      ...
      {/* ✅ Always show status update section */}
      ...
      <div className='flex justify-end gap-2'>
        <Button variant='outlined' color='error' onClick={() => router.push(`/${locale}/ticket/ticket-list`)}>
          Cancel
        </Button>
