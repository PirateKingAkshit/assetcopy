...
const TransferApprovalListTable = () => {
...
//     approvedDate: approved ? formatDate(approved.toISOString()) : '—'
//   }
// })
...
const fetchData = async (page = 1, pageSize = pagination.pageSize) => {
  try {
    ...
    if (response.data?.status === 200 && Array.isArray(response.data?.data)) {
      ...
      // Merge API pagination data with previous state
      setPagination(prev => ({
        ...prev,
        ...response.data.pagination,
        pageSize // Ensure pageSize from params is used
      }));
      setData(mappedData);
      setFilteredData(mappedData);
    } else {
  ...
  useEffect(() => {
    ...
    if (shouldReload) {
      setCookie('reloadTransferList', '', { maxAge: 0 })
        ...
        cellRenderer: params => (
          <div className='flex items-center gap-1 h-full'>
            <IconButton
              size='small'
              onClick={() => {
                router.push(`/${locale}/asset-managements/view-transferapproval/${params.data.id}`)
              }}
            >
  ...
  const handlePageChange = (event, newPage) => {
    setPagination(prev => ({ ...prev, currentPage: newPage }))
      ...
      const handlePageSizeChange = useCallback(newSize => {
        setCookie('transferapprovalPageSize', newSize.toString())
        setPagination(prev => ({
          ...prev,
          pageSize: newSize,
          currentPage: 1,
  ...
  return (
    <>
      ...
      <Card className='flex flex-col'>
        <CardHeader title='Transfer Approval' /> 
       

        <Divider />
        <div className='flex justify-between flex-col sm:flex-row p-5 gap-y-4'>
          <DebouncedInput value={globalFilter} onChange={handleFilterChange} placeholder='Search Transfer Approval' />
        </div>
          <div className='px-6'>
     ...
     <PaginationComponent
                  ...
                  storageKey='transferapprovalPageSize'
                />
      </Card>
    </>
  )
...
export default TransferApprovalListTable
